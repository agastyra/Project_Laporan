        ...
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('bkm.store') }}"
                    ...
                    <div class="form-group">
                        <label for=""
                            class="col-sm-3 col-form-label">Pilih Transaksi</label>
                        <select class="form-control text-light"
                            style="width:100%"
                            id="penjualan_memorial"
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#transaksi_penjualan_id').on('select2:select', function(e) {
                    ...
                    $.ajax({
                        url: '/accounting/getTransData/' + data.id,
                        dataType: 'json',
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#jurnal_memorial_id').on('select2:select', function(e) {
                    var data = e.params.data;
                    $.ajax({
                        url: '/accounting/getMemoData/' + data.id,
                        dataType: 'json',
                        success: function(response) {
                            $('#tanggal').val(response.tanggal);
        ...
        <script>
            $(document).ready(function() {
                ...
                function getMemoData() {
                    $.getJSON('/accounting/getMemoData/' + memoId, function(data) {
                        debet = data.debet;
                        kredit = data.kredit;
                        $('#debet').val(debet);
                        $('#kredit').val(kredit);
